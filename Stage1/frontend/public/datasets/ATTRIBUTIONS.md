...
## Digit training data — mnist/

- **mnist/clean.png** (900 tiles) — real handwritten digits from the **MNIST**
  training split, via the `mnist_png` PNG export.
  Source: https://github.com/myleott/mnist_png
  Original: http://yann.lecun.com/exdb/mnist/ — Yann LeCun, Corinna Cortes,
  Christopher J.C. Burges. MNIST is derived from NIST Special Database 3 and 1
  and is made freely available; the `mnist_png` export is distributed under the
  terms of its repository.
- **mnist/messy.png**, **mnist/noisy.png** — NOT separately collected. Derived
  from the clean tiles above by real augmentation (rotation, shear, stroke
  weight / blur, contrast loss, salt-and-pepper noise) in
  `Stage1/scripts/pack_digit_sprites.py`. `meta.json` records this and the UI
  labels them as derived.

## Attendance scenario — attendance/

- **attendance/register_sheet.png**, **attendance/arrivals/*.png** — generated by
  `Stage1/scripts/generate_attendance_assets.py`. Illustrated avatars drawn
  procedurally; no photographs of real people, so no third-party rights apply.
