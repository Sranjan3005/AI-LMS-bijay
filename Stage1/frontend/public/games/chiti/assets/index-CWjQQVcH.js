...
const SlidersVertical = createLucideIcon("sliders-vertical", __iconNode$5);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
function App() {
  ...
  const [activeLayout, setActiveLayout] = reactExports.useState(LAYOUT_A);
  const [isSimpleMode, setIsSimpleMode] = reactExports.useState(true);
  const [mission1Complete, setMission1Complete] = reactExports.useState(false);
  const [mission2Complete, setMission2Complete] = reactExports.useState(false);
  const [mission3Complete, setMission3Complete] = reactExports.useState(false);
  const [robotPos, setRobotPos] = reactExports.useState({ x: 0, y: 0 });
  const [robotMood, setRobotMood] = reactExports.useState("normal");
  const [score, setScore] = reactExports.useState(100);
  const [stepsCount, setStepsCount] = reactExports.useState(0);
  const [cumulativeReward, setCumulativeReward] = reactExports.useState(0);
  const [isPlaying, setIsPlaying] = reactExports.useState(false);
  const [playSpeed, setPlaySpeed] = reactExports.useState(150);
  const [trail, setTrail] = reactExports.useState([]);
  const [soundEnabled, setSoundEnabled] = reactExports.useState(true);
  const [lastActionName, setLastActionName] = reactExports.useState("");
  const [userRules, setUserRules] = reactExports.useState([
  ...
  const [rulesNeededCounter, setRulesNeededCounter] = reactExports.useState(4);
  const [infiniteLoopAlert, setInfiniteLoopAlert] = reactExports.useState(false);
  const stateVisitFrequency = reactExports.useRef({});
  const [qTable, setQTable] = reactExports.useState(
    () => Array.from({ length: 64 }, () => Array(4).fill(0))
  );
  const [epsilon, setEpsilon] = reactExports.useState(0.8);
  const [alpha2] = reactExports.useState(0.5);
  const [gamma] = reactExports.useState(0.9);
  const [rlEpisodesCount, setRlEpisodesCount] = reactExports.useState(0);
  const [rlSuccesses, setRlSuccesses] = reactExports.useState(0);
  const [showQValues, setShowQValues] = reactExports.useState(true);
  const [isTrainingFast, setIsTrainingFast] = reactExports.useState(false);
  ...
  const [impactOverlay, setImpactOverlay] = reactExports.useState(null);
  ...
  const isWall = reactExports.useCallback((x, y, layout2) => {
    return layout2.walls.some((w) => w.x === x && w.y === y);
  }, []);
  const isPuddle = reactExports.useCallback((x, y, layout2) => {
    return layout2.puddles.some((p) => p.x === x && p.y === y);
  }, []);
  const resetSimulation = reactExports.useCallback(() => {
    setRobotPos({ x: 0, y: 0 });
    setRobotMood("normal");
    setScore(100);
    setStepsCount(0);
  ...
  const checkCondition = reactExports.useCallback((condition, x, y, layout2) => {
    switch (condition) {
      case "always":
        return true;
      case "right_clear": {
        const nextX = x + 1;
        return nextX < GRID_SIZE && !isWall(nextX, y, layout2);
      }
      case "down_clear": {
        const nextY = y + 1;
        return nextY < GRID_SIZE && !isWall(x, nextY, layout2);
      }
      case "wall_right": {
        const nextX = x + 1;
        return nextX >= GRID_SIZE || isWall(nextX, y, layout2);
      ...
      case "in_puddle":
        return isPuddle(x, y, layout2);
      default:
        return false;
    }
  }, [isWall, isPuddle]);
  const executeRuleStep = reactExports.useCallback(() => {
    let selectedAction = "";
  ...
  const executeRlStep = reactExports.useCallback(() => {
    ...
    const updatedQ = qTable[s][actionIndex] + alpha2 * (r + gamma * maxNextQ - qTable[s][actionIndex]);
  ...
  }, [robotPos, qTable, activeLayout, isWall, isPuddle, epsilon, alpha2, gamma]);
  const trainFast = () => {
    ...
    setTimeout(() => {
      ...
      for (let ep = 0; ep < 100; ep++) {
        ...
        while (!episodeFinished && steps < 120) {
          ...
          tempQTable[s][aIndex] = tempQTable[s][aIndex] + alpha2 * (r + gamma * maxNextQ - tempQTable[s][aIndex]);
          x = nextX;
          y = nextY;
          steps++;
        }
  ...
  reactExports.useEffect(() => {
    ...
    if (activeTab === "rl" && showQValues) {
      for (let s = 0; s < 64; s++) {
        ...
        if (Math.abs(maxVal) > 0.01) {
          ...
          const intensity = Math.min(Math.abs(maxVal) / 80, 1);
          const color2 = maxVal > 0 ? `rgba(16, 185, 129, ${0.15 + intensity * 0.7})` : `rgba(239, 68, 68, ${0.15 + intensity * 0.7})`;
    ...
    if (impactOverlay) {
      ...
      ctx.fillStyle = impactOverlay.color.includes("red") ? "#ef4444" : impactOverlay.color.includes("blue") ? "#3b82f6" : impactOverlay.color.includes("green") ? "#10b981" : "#cbd5e1";
  ...
  const changeRuleCondition = (id2, condition) => {
    setUserRules((prev) => prev.map((rule) => rule.id === id2 ? { ...rule, condition } : rule));
    resetSimulation();
  };
  const changeRuleAction = (id2, action) => {
    setUserRules((prev) => prev.map((rule) => rule.id === id2 ? { ...rule, action } : rule));
    resetSimulation();
  };
  const deleteRule = (id2) => {
    setUserRules((prev) => prev.filter((rule) => rule.id !== id2));
    resetSimulation();
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-[600px] bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("header", { className: "h-16 bg-slate-800 border-b border-slate-700 flex items-center justify-between px-6 shrink-0 sticky top-0 z-50 shadow-md", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-[1600px] w-full mx-auto flex items-center justify-between gap-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-8 h-8 bg-indigo-500 rounded flex items-center justify-center font-bold text-white shadow-lg shadow-indigo-500/20", children: "🤖" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("h1", { className: "text-lg md:text-xl font-display font-bold tracking-tight text-white flex items-center gap-2", children: [
          "Robot Intelligence Lab",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "hidden sm:inline text-slate-400 font-normal text-xs bg-slate-900/85 px-2.5 py-1 rounded-full border border-slate-700/65", children: "CBSE Grade 6-8 AI Explorer" })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-indigo-950/80 px-4 py-1.5 rounded-full border border-indigo-500/30 flex items-center gap-2 shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-indigo-200 text-xs md:text-sm italic font-medium leading-none", children: [
        '"Intelligence is ',
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-bold text-indigo-400 font-display", children: "EARNED" }),
        ' from experience."'
      ] }) })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-slate-900 border-b border-slate-700 py-3 px-6 shrink-0 shadow-sm", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-[1600px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs font-bold text-indigo-400 uppercase tracking-wider font-display shrink-0", children: "Choose Sandbox Level:" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex bg-slate-950 p-1 rounded-xl border border-slate-700 w-full sm:w-auto", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              className: `flex-1 sm:flex-none px-4 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${isSimpleMode ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/10" : "text-slate-400 hover:text-slate-200"}`,
              children: "🎒 Class 6-8 Playful Mode"
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              className: `flex-1 sm:flex-none px-4 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${!isSimpleMode ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/10" : "text-slate-400 hover:text-slate-200"}`,
              children: "🔬 Lab Mode (Advanced)"
            }
          )
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between sm:justify-start gap-3 text-xs bg-slate-950/80 px-4 py-2 rounded-xl border border-slate-700 w-full md:w-auto shadow-inner", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-amber-400 text-sm", children: "⭐" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-300 font-medium", children: isSimpleMode ? "Lab Missions Completed:" : "Milestone Badges:" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "font-mono font-black text-white bg-indigo-950/80 px-2.5 py-0.5 rounded border border-indigo-500/20 text-xs shadow-lg shadow-indigo-500/5", children: [
          [mission1Complete, mission2Complete, mission3Complete].filter(Boolean).length,
          " / 3"
        ] })
      ] })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-slate-900/60 border-b border-slate-700 py-3 px-6 shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-w-[1600px] mx-auto flex items-center justify-center gap-2 sm:gap-3 flex-wrap", children: ["① Intro & Theory", "② The Lab", "③ Quiz"].map((label, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        onClick: () => setPage(i),
        className: `px-3 sm:px-5 py-2 rounded-full text-xs sm:text-sm font-bold transition-all ${page === i ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/20" : "bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700"}`,
        children: label
      },
      i
    )) }) }),
    page === 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "flex-1 max-w-[1000px] w-full mx-auto p-4 md:p-6 flex flex-col gap-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-gradient-to-br from-indigo-950 to-slate-900 border border-indigo-500/30 rounded-2xl p-6 md:p-8 text-center shadow-xl", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-5xl mb-3", children: "🤖" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-2xl md:text-3xl font-display font-bold text-white mb-3", children: "Meet Chitti — a robot who learns." }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-sm md:text-base text-slate-300 max-w-2xl mx-auto leading-relaxed", children: [
          "You're about to discover the single biggest idea in AI: the difference between a robot that ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { className: "text-indigo-300", children: "follows rules" }),
          " you write, and one that ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { className: "text-emerald-300", children: "learns from experience" }),
          ". Chitti has to reach his charging station 🔋 across a tricky 8×8 world of walls 🧱 and puddles 💧."
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 border border-slate-700 rounded-xl p-5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 mb-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SlidersVertical, { className: "w-5 h-5 text-indigo-400" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-display font-bold text-indigo-300", children: "Mode 1 · Fixed Rules 📝" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs text-slate-400 leading-relaxed", children: [
            "You hand Chitti a checklist of ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: "if-then" }),
            " rules. They work perfectly on the simple map… until the world changes and the rules pile up and break. Writing a rule for every possible situation is impossible."
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 border border-slate-700 rounded-xl p-5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 mb-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Zap, { className: "w-5 h-5 text-emerald-400" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-display font-bold text-emerald-300", children: "Mode 2 · Learning Brain 🧠" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs text-slate-400 leading-relaxed", children: [
            'Now Chitti gets no rules — only rewards ⭐ for good moves and penalties ✗ for bad ones. Through trial and error he builds a "brain map" and figures out the maze ',
            /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: "by himself" }),
            ". That's Reinforcement Learning."
          ] })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 border border-slate-700 rounded-xl p-5", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-xs font-bold uppercase text-slate-300 tracking-wider mb-3 flex items-center gap-2", children: "🎯 What you'll do in the Lab" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("ul", { className: "grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs text-slate-300", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("li", { className: "flex items-center gap-2 bg-slate-900 rounded-lg p-3 border border-slate-700", children: "🏆 Guide Chitti with rules on Map 1" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("li", { className: "flex items-center gap-2 bg-slate-900 rounded-lg p-3 border border-slate-700", children: "🌋 Watch the rules break on Map 2" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("li", { className: "flex items-center gap-2 bg-slate-900 rounded-lg p-3 border border-slate-700", children: "🧠 Train his brain to solve it himself" })
        ] })
      ] })
    ] }),
    page === 1 && /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "flex-1 max-w-[1600px] w-full mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lg:col-span-5 flex flex-col gap-6", children: [
        isSimpleMode && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          motion.div,
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-3xl shrink-0 select-none animate-bounce", children: "💡" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-bold text-indigo-300 uppercase tracking-widest font-mono block", children: "Professor Byte Says:" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-200 mt-1 leading-relaxed font-medium", children: getTeacherMessage() })
              ] })
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 rounded-xl p-5 border border-slate-700 shadow-xl flex flex-col gap-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("h2", { className: "text-md font-display font-semibold text-slate-200 flex items-center gap-2", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-mono text-indigo-300 uppercase tracking-wider bg-slate-900 px-2 py-0.5 rounded border border-slate-700", children: "Live Preview" })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative mx-auto bg-slate-950 rounded-xl overflow-hidden border border-slate-700 shadow-inner flex justify-center items-center", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "canvas",
              {
                id: "mazeCanvas",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(AnimatePresence, { children: infiniteLoopAlert && activeTab === "rules" && /* @__PURE__ */ jsxRuntimeExports.jsxs(
              motion.div,
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(TriangleAlert, { className: "w-12 h-12 text-red-500 mb-3 animate-bounce" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-lg font-display font-bold text-red-400", children: "🚨 Rule Failure Loop!" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-300 max-w-xs mt-1.5", children: "Chitti is repeating the same steps infinitely! Hand-written rules cannot handle the new block." }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      ...
                      children: "Dismiss & Write More Rules"
                    }
                  )
                ]
              }
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-900 rounded-xl p-3 border border-slate-700 grid grid-cols-2 gap-3 text-xs", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-400 block text-[10px] uppercase font-bold tracking-wider", children: "Chitti's Location" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "font-mono font-bold text-white text-sm", children: [
                "Col ",
                robotPos.x + 1,
                " , Row ",
                robotPos.y + 1
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-400 block text-[10px] uppercase font-bold tracking-wider", children: "Simulation Steps" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-mono font-bold text-white text-sm", children: stepsCount })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-400 block text-[10px] uppercase font-bold tracking-wider", children: "Score (Starts at 100)" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "font-mono font-bold text-indigo-400 text-sm", children: [
                score,
                " pts"
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-400 block text-[10px] uppercase font-bold tracking-wider", children: "Net Reward" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `font-mono font-bold text-sm ${cumulativeReward >= 0 ? "text-emerald-400" : "text-rose-400"}`, children: cumulativeReward })
            ] })
          ] })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lg:col-span-7 flex flex-col gap-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 rounded-xl p-4 border border-slate-700 shadow-lg flex flex-col gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("h3", { className: "text-xs font-bold uppercase text-slate-300 tracking-wider flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SlidersVertical, { className: "w-4 h-4 text-indigo-400" }),
            " Controls"
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(RotateCcw, { className: "w-3.5 h-3.5 text-slate-400" }),
                  "Reset Chitti"
                ]
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronRight, { className: "w-3.5 h-3.5 text-slate-400" }),
                  "Manual Step"
                ]
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                disabled: robotPos.x === activeLayout.goal.x && robotPos.y === activeLayout.goal.y,
                className: `flex-1 py-2 px-4 text-xs font-bold rounded-lg flex items-center justify-center gap-1.5 transition-all ${isPlaying ? "bg-rose-600 hover:bg-rose-500 text-white shadow-md shadow-rose-500/15 animate-pulse" : "bg-indigo-600 hover:bg-indigo-500 text-white shadow-md shadow-indigo-600/15"}`,
                children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between gap-4 bg-slate-900 p-2.5 rounded-lg border border-slate-700 text-xs", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-400 shrink-0 font-bold tracking-wider uppercase text-[10px]", children: "Tick Speed:" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "range",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-slate-300 font-mono shrink-0", children: [
              playSpeed,
              "ms"
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-[10px] font-bold text-slate-400 tracking-wider uppercase", children: "Choose Environment Map:" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  onClick: () => {
                    setActiveLayout(LAYOUT_A);
                    resetSimulation();
                  },
                  className: `flex-1 py-2 px-3 rounded-lg text-xs font-semibold border transition-all flex items-center justify-center gap-1.5 ${activeLayout.id === "layout_a" ? "bg-indigo-600/15 border-indigo-500 text-indigo-300 font-bold shadow-md shadow-indigo-600/5" : "bg-slate-900 border-slate-700 text-slate-300 hover:bg-slate-800"}`,
                  children: "🗺️ Map 1: Simple World"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  onClick: () => {
                    setActiveLayout(LAYOUT_B);
                    resetSimulation();
                  },
                  className: `flex-1 py-2 px-3 rounded-lg text-xs font-semibold border transition-all flex items-center justify-center gap-1.5 ${activeLayout.id === "layout_b" ? "bg-indigo-600/15 border-indigo-500 text-indigo-300 font-bold shadow-md shadow-indigo-600/5" : "bg-slate-900 border-slate-700 text-slate-300 hover:bg-slate-800"}`,
                  children: "🌋 Map 2: Changed World"
                }
              )
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-[11px] text-slate-400 bg-slate-950 p-2.5 rounded-lg border border-slate-700 mt-1 leading-relaxed", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Map Status:" }),
              " ",
              activeLayout.description
            ] })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 border border-slate-700 rounded-xl p-1 flex shrink-0 shadow-lg", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              onClick: () => {
              ...
              className: `flex-1 py-2 rounded-lg text-xs font-semibold tracking-tight transition-all flex items-center justify-center gap-2 ${activeTab === "rules" ? "bg-indigo-600 text-white font-bold shadow-md shadow-indigo-600/15" : "text-slate-400 hover:text-white"}`,
              children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              className: `flex-1 py-2 rounded-lg text-xs font-semibold tracking-tight transition-all flex items-center justify-center gap-2 ${activeTab === "rl" ? "bg-indigo-600 text-white font-bold shadow-md shadow-indigo-600/15" : "text-slate-400 hover:text-white"}`,
              children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 bg-slate-800 rounded-xl border border-slate-700 p-5 shadow-xl min-h-[460px] flex flex-col gap-4", children: [
          activeTab === "rules" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-4 flex-1", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "border-b border-slate-700 pb-3", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "h-5 w-5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-xs font-black rounded-lg flex items-center justify-center", children: "1" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-md font-display font-bold text-indigo-400", children: isSimpleMode ? "📝 Teach Chitti with Simple IF-THEN Rules" : "Program Chitti using Fixed Rules" })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-400 mt-1", children: isSimpleMode ? "Create a checklist of rules. Chitti will read them from top to bottom to make his move!" : "Design a sequence of logical conditional blocks. Chitti will check them from top to bottom." })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-2 bg-slate-900 p-3 rounded-xl border border-slate-700 max-h-[280px] overflow-y-auto", children: [
              userRules.map((rule, idx) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 w-full sm:w-auto", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-mono text-[10px] text-indigo-400 font-bold select-none shrink-0 bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20", children: "IF" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs(
                        "select",
                        {
                          ...
                          children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "always", children: "Always (Default fallback)" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "right_clear", children: "Path to Right ➡️ is CLEAR" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "down_clear", children: "Path Below ⬇️ is CLEAR" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "wall_right", children: "Wall/Obstacle is on Right 🧱" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "wall_down", children: "Wall/Obstacle is Below 🧱" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "in_puddle", children: "Chitti is in a Muddy Puddle 💧" })
                          ]
                        }
                      )
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-start", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5", children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-mono text-[10px] text-slate-500 font-bold uppercase", children: "THEN" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsxs(
                          "select",
                          {
                            value: rule.action,
                            ...
                            children: [
                              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "MOVE_RIGHT", children: "Go Right ➡️" }),
                              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "MOVE_DOWN", children: "Go Down ⬇️" }),
                              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "MOVE_LEFT", children: "Go Left ⬅️" }),
                              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "MOVE_UP", children: "Go Up ⬆️" })
                            ]
                          }
                        )
                      ] }),
                      userRules.length > 1 && /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "button",
                        {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-slate-900 rounded-xl p-4 border border-slate-700 mt-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-3", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Lightbulb, { className: "w-5 h-5 text-indigo-400 shrink-0 mt-0.5" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("h4", { className: "text-xs font-display font-bold text-indigo-300 uppercase tracking-wider", children: "The Educational Discovery:" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs text-slate-300 leading-relaxed mt-1 font-sans", children: [
                  "Notice how the default rules lead Chitti to the goal in ",
                  /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Map 1 (Simple World)" }),
                  " perfectly. But click ",
                  /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: '"Map 2 (Changed World)"' }),
                  " and press Auto-Run. Chitti will get trapped because the hardcoded rules did not expect the giant central wall!"
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-3 flex items-center gap-2.5 bg-slate-950 p-2.5 rounded-lg border border-slate-700", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] uppercase font-bold text-slate-400", children: "Rules needed to prevent loop:" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1 font-mono font-black text-sm", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-indigo-400", children: "4" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-500", children: "➔" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `transition-all ${activeLayout.id === "layout_b" ? "text-amber-400 scale-110" : "text-slate-400"}`, children: "12" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-500", children: "➔" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-600", children: "23" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-500", children: "➔" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-red-500 animate-pulse text-lg", children: "∞" })
                  ] })
                ] })
              ] })
            ] }) })
          ] }),
          activeTab === "rl" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-4 flex-1", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "border-b border-slate-700 pb-3 flex flex-col sm:flex-row justify-between sm:items-center gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "h-5 w-5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-xs font-black rounded-lg flex items-center justify-center", children: "2" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-md font-display font-bold text-indigo-400", children: isSimpleMode ? "🧠 Teach Chitti via Trial & Error (ML)" : "Train Chitti via Trial & Error" })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-400 mt-1", children: isSimpleMode ? "Chitti creates a 'Mental Map' inside his brain to remember which steps were rewarding." : "Chitti maintains a Q-Table map to remember which steps were rewarding." })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "flex items-center gap-2 bg-slate-950 px-2.5 py-1 rounded-lg border border-slate-700 cursor-pointer text-[11px] hover:bg-slate-900 transition-colors self-start shrink-0", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "input",
                  {
                    type: "checkbox",
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: isSimpleMode ? "👀 Show Thought Arrows" : "Show Brain Arrows" })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-900 p-3 rounded-xl border border-slate-700 flex flex-col gap-1.5 justify-center", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] text-slate-400 uppercase tracking-widest block font-bold", children: isSimpleMode ? "🎲 Curiosity (Adventure)" : "Curiosity (Epsilon)" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "font-mono font-bold text-amber-400 text-md", children: [
                    Math.round(epsilon * 100),
                    "%"
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] text-slate-400 italic", children: epsilon > 0.5 ? "🎲 Exploring" : "🧠 Smart Move" })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "input",
                  {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-900 p-3 rounded-xl border border-slate-700 flex flex-col justify-center", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] text-slate-400 uppercase tracking-widest block font-bold", children: isSimpleMode ? "⚡ Practice Runs Completed" : "Lessons Completed" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "font-mono font-black text-2xl text-emerald-400 mt-1", children: [
                  rlEpisodesCount,
                  " ",
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-slate-500 font-normal", children: "runs" })
                ] })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col sm:flex-row gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(RotateCcw, { className: "w-3.5 h-3.5 text-slate-500" }),
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(TrendingUp, { className: "w-4 h-4 text-indigo-200" }),
                    isTrainingFast ? "Simulating Fast..." : isSimpleMode ? "⚡ Learn 100 Times Instantly!" : "Train 100 Episodes Instantly!"
                  ]
                }
              )
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-slate-900 rounded-xl p-4 border border-slate-700 mt-auto leading-relaxed", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-3", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { className: "w-5 h-5 text-indigo-400 shrink-0 mt-0.5" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("h4", { className: "text-xs font-display font-bold text-indigo-300 uppercase tracking-wider", children: "The AI Lab Breakthrough:" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs text-slate-300 mt-1", children: [
                  "Press ",
                  /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: '"Train 100 Episodes Instantly!"' }),
                  " 1 or 2 times, then press ",
                  /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Auto-Run" }),
                  " on Map 2. Watch Chitti immediately find the perfect path bypass without hitting a single brick!"
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-indigo-300 font-bold mt-2 font-sans", children: "💡 Notice the glowing directional arrows appearing on the grid! That is the physical brain map stored in the Q-Table." })
              ] })
            ] }) })
          ] })
        ] }),
        isSimpleMode && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          motion.div,
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5 border-b border-slate-700 pb-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-md", children: "🎯" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-xs font-bold uppercase text-slate-300 tracking-wider", children: "Chitti's Training Missions" })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2.5", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `h-4.5 w-4.5 rounded flex items-center justify-center border transition-all shrink-0 ${mission1Complete ? "bg-emerald-500/20 border-emerald-500 text-emerald-400" : "border-slate-600 bg-slate-900/50"}`, children: mission1Complete ? "✓" : "" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `transition-all ${mission1Complete ? "text-slate-500 line-through" : "text-slate-300 font-medium"}`, children: "🏆 Rules guide Chitti on Map 1" })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2.5", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `h-4.5 w-4.5 rounded flex items-center justify-center border transition-all shrink-0 ${mission2Complete ? "bg-emerald-500/20 border-emerald-500 text-emerald-400" : "border-slate-600 bg-slate-900/50"}`, children: mission2Complete ? "✓" : "" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `transition-all ${mission2Complete ? "text-slate-500 line-through" : "text-slate-300 font-medium"}`, children: "🌋 Trigger a loop on Map 2" })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2.5", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `h-4.5 w-4.5 rounded flex items-center justify-center border transition-all shrink-0 ${mission3Complete ? "bg-emerald-500/20 border-emerald-500 text-emerald-400" : "border-slate-600 bg-slate-900/50"}`, children: mission3Complete ? "✓" : "" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `transition-all ${mission3Complete ? "text-slate-500 line-through" : "text-slate-300 font-medium"}`, children: "🧠 ML Brain finds the path" })
                ] })
              ] })
            ]
          }
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "col-span-1 lg:col-span-12 bg-slate-800 rounded-xl p-5 border border-slate-700 shadow-xl flex flex-col gap-3.5", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("h3", { className: "text-xs font-display font-semibold uppercase text-slate-300 tracking-wider flex items-center gap-1.5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(BookOpen, { className: "w-4 h-4 text-indigo-400" }),
          isSimpleMode ? "🎒 Middle School AI Glossary" : "CBSE COMPUTER SCIENCE GLOSSARY"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-xs leading-relaxed", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-900 p-2.5 rounded-lg border border-slate-700", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-indigo-300 font-semibold block mb-0.5 font-display", children: isSimpleMode ? "🤖 Chitti (The Learner)" : "🤖 Agent (Learner)" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-slate-400", children: isSimpleMode ? "The robot or AI that is trying to solve the puzzle. Here, Chitti is the learner!" : "The active system that makes decisions. Here, Chitti is the agent!" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-900 p-2.5 rounded-lg border border-slate-700", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-amber-400 font-semibold block mb-0.5 font-display", children: isSimpleMode ? "🗺️ Play Grid (The Environment)" : "🗺️ Environment" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-slate-400", children: isSimpleMode ? "The world Chitti lives in—our 8x8 grid filled with brick walls and mud puddles." : "The surrounding world. Here, the 8x8 Grid Arena with obstacles is the environment." })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-900 p-2.5 rounded-lg border border-slate-700", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-indigo-300 font-semibold block mb-0.5 font-display", children: isSimpleMode ? "📍 Position (The State)" : "📍 State (Condition)" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-slate-400", children: isSimpleMode ? "Where Chitti is standing right now on the grid (like Column 1, Row 1)." : "The agent's current situation. Chitti's current grid position is his state." })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-900 p-2.5 rounded-lg border border-slate-700", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-emerald-400 font-semibold block mb-0.5 font-display", children: isSimpleMode ? "🎁 Star vs Splash (Feedback)" : "🎁 Reward / Penalty" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-slate-400", children: isSimpleMode ? "+100 Gold Stars for reaching the green battery, and minus points for hitting walls or splashing mud!" : "Numerical feedback (+100 for goals, negative values for muddy splashes or wall bumps) teaching the agent." })
          ] })
        ] })
      ] })
    ] }),
    page === 2 && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "bg-gradient-to-r from-slate-900 via-indigo-950/80 to-slate-900 border-y border-slate-700 py-8 px-6 text-center relative overflow-hidden shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-4xl mx-auto relative z-10 flex flex-col items-center", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-2 bg-indigo-500/10 rounded-full border border-indigo-500/20 mb-3 text-indigo-400", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Award, { className: "w-8 h-8 animate-pulse" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("h2", { className: "text-xl md:text-2xl font-display font-bold tracking-tight text-white max-w-3xl leading-snug", children: [
          '"Rules are written by hand and break easily.',
          /* @__PURE__ */ jsxRuntimeExports.jsx("br", { className: "hidden md:inline" }),
          " Intelligence is ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-indigo-400 font-display font-bold", children: "EARNED from experience" }),
          '."'
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-400 max-w-2xl mt-3 leading-relaxed", children: "This is the fundamental reason why modern AI like self-driving cars, drone navigations, and robotics do not use rigid 'if-then' programming. Instead, they use Machine Learning models that trial-and-error millions of times inside secure simulators to build smart intelligence." })
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "max-w-4xl w-full mx-auto p-4 md:p-6 mb-12", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 rounded-xl border border-slate-700 p-6 shadow-xl flex flex-col gap-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2.5 border-b border-slate-700 pb-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(GraduationCap, { className: "w-6 h-6 text-indigo-400" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-md font-display font-semibold text-slate-200", children: "CBSE Assessment Corner" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-400", children: "Test your learning outcomes and earn a virtual AI badge!" })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-6", children: CBSE_QUIZ_QUESTIONS.map((q) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-900 p-4 rounded-lg border border-slate-700/80 flex flex-col gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-xs font-bold text-indigo-400 uppercase tracking-wider font-mono", children: [
            "Question ",
            q.id
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs font-bold text-slate-200", children: q.question }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-2 mt-1", children: q.options.map((opt, optIdx) => {
            ...
            return /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "h-4 w-4 rounded-full border border-current flex items-center justify-center shrink-0 text-[10px] font-bold", children: String.fromCharCode(65 + optIdx) }),
          ...
          quizSubmitted && selectedAnswers[q.id] !== void 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `mt-2 p-3 rounded-lg text-xs leading-relaxed border ${selectedAnswers[q.id] === q.correctIndex ? "bg-emerald-500/10 border-emerald-500/20 text-slate-300" : "bg-amber-500/10 border-amber-500/20 text-slate-300"}`, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "font-bold flex items-center gap-1.5 text-[11px] mb-0.5", children: [
              selectedAnswers[q.id] === q.correctIndex ? /* @__PURE__ */ jsxRuntimeExports.jsx(CircleCheck, { className: "w-3.5 h-3.5 text-emerald-400" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(TriangleAlert, { className: "w-3.5 h-3.5 text-amber-400" }),
              selectedAnswers[q.id] === q.correctIndex ? "Correct Answer!" : "Insight:"
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-slate-400", children: q.explanation })
          ] })
        ] }, q.id)) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-slate-700 pt-5", children: !quizSubmitted ? /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            onClick: submitQuiz,
        ...
        ) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col sm:flex-row items-center gap-4 w-full justify-between", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-10 w-10 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Award, { className: "w-5 h-5 animate-pulse" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] text-slate-400 block uppercase font-bold", children: "Your Results" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-sm font-extrabold text-white", children: [
                "Score: ",
                quizScore,
                " / ",
                CBSE_QUIZ_QUESTIONS.length,
                " Correct"
              ] })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2 w-full sm:w-auto", children: [
            ...
            quizScore === CBSE_QUIZ_QUESTIONS.length && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "py-2 px-4 bg-emerald-500/15 border border-emerald-500/35 text-emerald-300 text-xs font-semibold rounded-lg flex items-center gap-1", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { className: "w-3.5 h-3.5 text-emerald-400" }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-[1600px] w-full mx-auto px-6 py-6 flex justify-between items-center gap-4 shrink-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
      ...
      page < 2 ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("footer", { className: "mt-auto border-t border-slate-700 bg-slate-800 py-6 px-6 text-slate-400 text-xs shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-[1600px] mx-auto flex flex-col sm:flex-row justify-between items-center gap-3 text-center sm:text-left", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-bold text-slate-300", children: "CBSE AI Subject Code 417 - Interactive Activity" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px] text-slate-500 mt-0.5", children: "Designed to align with standard middle school ML curricula across Indian board standards." })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px]", children: "© 2026 AI Studio Build. Self-contained pedagogical game." })
    ] }) })
  ] });
}
