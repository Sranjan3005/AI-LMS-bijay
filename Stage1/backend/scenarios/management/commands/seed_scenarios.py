...
SCENARIOS_DATA = [
    ...
    },
    {
        ...
        'order':      8,
        'is_active':  True,
        'challenge':  'Predict the number of comments a post will get based on how many likes it has.',
        'takeaway':   'Generally, more likes means more comments — but an algorithm quirk, a bot attack, or a disabled comment section can shatter that rule. A model only knows the columns you give it.',
        'try_it_out': 'Open Instagram, YouTube or any app, pick 10 random posts, and log the likes (X) vs comments (Y). Does your model\'s rule hold?',
        'story':      'Every creator — from your favourite YouTuber to your school\'s Instagram page — wants to know what makes a post buzz. Likes are easy to get, but COMMENTS mean people actually cared enough to type something. If likes could predict comments, a creator would know which posts are truly connecting. That is a prediction problem, and regression is built for it.',
        'data_story': 'You collect the data yourself: scroll a feed, and for each post write down two numbers — its likes and its comments. Each row is one post. It feels objective… but social media data is full of traps. A post can be boosted by an algorithm, flooded by bots, or have its comments switched off — and none of that shows up in your two columns.',
        'outcome_guide': 'Look at the line: does more likes really mean more comments? Check how tightly the dots hug it — social data is usually messy, so expect scatter. Then hunt for the weird dots far from the line: a post with tons of likes but no comments (comments disabled), or few likes but a flood of comments (bots). Those outliers are the whole lesson — the model blames "likes" because that is the only column it can see.',
        'guide_steps': [
            'Pick a variant and read its "what to watch for" note.',
            'Preview the data — find the post with the most likes and check its comments.',
            'Train the model and read its rule: about how many comments per 1000 likes?',
            'Find the dot furthest from the line. What real-life reason (bots? disabled comments?) could explain it?',
        ],
        'variants': [
            {'name': 'perfect', 'label': 'Viral Content',     'order': 1,
             'description': 'A clean batch of genuine posts where more likes really does mean more comments.',
             'watch_for': 'A clear upward line — the honest baseline. Notice social data is still bumpier than, say, temperature vs sales.'},
            {'name': 'disabled','label': 'Disabled Comments', 'order': 2,
             'description': 'Includes a mega-post with a million likes but ZERO comments — the creator switched comments off.',
             'watch_for': 'One dot sits flat on the floor at huge likes. Watch it drag the line down and wreck predictions for every normal post — a hidden setting your two columns never captured.'},
            {'name': 'bot',     'label': 'The Bot Attack',    'order': 3,
             'description': 'A post with barely any likes but thousands of spam comments from bots.',
             'watch_for': 'A dot shoots up on the comments axis with almost no likes. The model can\'t tell real engagement from bot spam — rubbish in the data becomes a rubbish rule.'},
        ],
    },
    ...
    },
    {
        ...
        'try_it_out': 'Write your name in different styles — normal writing, cursive, block letters — and see which style the AI reads best.',
        ...
        'data_story': 'OCR engines are trained on millions of text samples — but mostly neat, normal writing text. That training history is why they ace normal writing and stumble on your grandmother\'s cursive. The data a model grew up on decides who it serves best: a lesson that applies to every AI, not just this one.',
        'outcome_guide': 'Compare the decoded text against what was actually written, per person. Normal writing usually wins and joined cursive loses — because OCR must first CUT the writing into separate letters, and cursive refuses to be cut. When one person\'s writing scores worst, that\'s not an insult — it\'s a demonstration that models inherit the limits of their training data.',
        'guide_steps': [
            'Write one short word (like your name) in neat NORMAL WRITING and run the decoder.',
        ...
        'variants': [
            {'name': 'normal',   'label': 'Normal Writing', 'order': 1,
             'description': 'Clean, separated normal handwriting — each letter standing alone.',
